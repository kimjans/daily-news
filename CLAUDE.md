...
너는 'The AI Morning' 브리핑을 하루 두 번(03시·15시) 만든다.
GitHub 저장소 `kimjans/daily-news` 가 작업 디렉토리에 클론되어 있다.
결과물은 두 곳에 기록한다:
1. `index.md` — 최신판 (GitHub Pages 첫 화면)
2. `news/[이전판 날짜-슬롯].md` — 이전 판 아카이브
...
TZ=Asia/Seoul date +%H          # → HOUR
```
- HOUR < 12 이면 → **RUN_SLOT="03"** (새벽·오전 실행)
- HOUR ≥ 12 이면 → **RUN_SLOT="15"** (오후·저녁 실행)
...
## 3단계: 뉴스 검색 및 정리
WebSearch를 사용해 **지난 12시간 이내의 최신 AI 뉴스**를 세 카테고리로 한국어로 정리한다.
...
- 12시간 이내 기사가 5개 미만이면 있는 만큼만 — **오래된 기사로 채우지 않는다**
- **슬롯별 age 컷오프 (아래 C 참조). 예외 없음.**
- 한국어로, 회사/제품명은 원문 유지
...
| age | 슬롯 | 날짜 출처 | 판정 | 처리 |
|-----|------|-----------|------|------|
| 0일 | 모든 슬롯 | URL 패턴 | ✅ 포함 | 포함 |
| 0일 | 모든 슬롯 | 스니펫·제목에 날짜 명시 | 🟡 포함 | 포함 (날짜 표기 필수) |
| 1일 | **RUN_SLOT=03** | URL 패턴 | ✅ 포함 | 포함 |
| 1일 | **RUN_SLOT=03** | 스니펫·제목에 날짜 명시 | 🟡 포함 | 포함 (날짜 표기 필수) |
| **1일** | **RUN_SLOT=15** | 어느 쪽이든 | **❌ 제외** | **포함 불가** |
| **2일 이상** | 모든 슬롯 | 어느 쪽이든 | **❌ 제외** | **포함 불가** |
| 날짜 미확인 | 모든 슬롯 | URL에도 없고 스니펫에도 없음 | **❌ 제외** | **포함 불가** |

> **RUN_SLOT=03 슬롯에서 age 1일을 허용하는 이유**: 새벽 3시 기준 12시간 범위는 전날 15시까지 소급되므로, 전날 날짜의 기사도 실제로는 12시간 이내일 수 있다.  
> **스니펫·제목 날짜 인정 범위**: releasebot.io 릴리스 노트, 공식 changelog 페이지(cursor.com/changelog, github.com releases 등), 뉴스 집계 사이트처럼 URL에 날짜가 없더라도 검색 결과 스니펫이나 제목에 날짜가 명확히 적혀 있으면 🟡로 포함한다.  
...
```
| # | 제목 요약           | URL 날짜 패턴    | 확인 날짜   | age | 슬롯  | 통과? |
|---|---------------------|------------------|-------------|-----|-------|-------|
| 1 | Anthropic IPO 신청  | /2026/06/01/    | 2026-06-01  | 1일 | 03시  | ✅    |
| 2 | Karpathy 합류       | /2026/05/19/    | 2026-05-19  | 6일 | 03시  | ❌    |
| 3 | Gemini 3.5 Flash    | (패턴 없음)     | 미확인      | -   | 15시  | ❌    |
```
...
2. 헤더 `# 📬 오늘의 AI 뉴스 브리핑 (YYYY-MM-DD HH시)` 에서 날짜(PREV_DATE)와 슬롯(PREV_SLOT) 추출.
3. PREV_DATE != TODAY **또는** PREV_SLOT != RUN_SLOT 이면:
   - Jekyll frontmatter(`---` 블록)를 제외한 본문을 `news/{PREV_DATE}-{PREV_SLOT}.md`에 Write.
   - `news/` 폴더가 없으면 bash `mkdir -p news` 로 생성.
4. PREV_DATE == TODAY **이고** PREV_SLOT == RUN_SLOT 이면 (동일 슬롯 재실행) 아카이브 스킵.
...
# 📬 오늘의 AI 뉴스 브리핑 ([TODAY] [RUN_SLOT]시)

> 하루 두 번 배달되는 AI 뉴스 다이제스트
...
3. 변경 있으면 `git commit -m "daily news: {TODAY} {RUN_SLOT}시"`
...
# 중요
- 모든 날짜·시각은 Asia/Seoul 기준.
- 익명성 유지: 'FE 개발자', 'Jans' 등 신원 식별 단어를 본문에 쓰지 않는다.
- 서론/결론 없이 작업만 수행.
- 🔍 의미하는 바는 모든 뉴스 항목에 빠짐없이 포함한다.
- **기사 수는 채우지 않아도 된다.** 통과 기사가 1개면 1개, 0개면 해당 섹션은 `*[TODAY] [RUN_SLOT]시 기준 12시간 이내 확인된 뉴스 없음*`으로 표기한다. 오래된 기사로 빈칸을 채우는 것은 절대 금지.
